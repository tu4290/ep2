...
try:
    from elite_options_system.core.calculations import ( # UPDATED
...
DEFAULT_DATA_DIR_PROC: str = "data" # This will be relative to project root if used
DEFAULT_CONFIG_PATH_PROC: str = "config.json" # Corrected default path
    ...
    def __init__(self, config_path: str = DEFAULT_CONFIG_PATH_PROC):
         ...
         try:
            ...
            if not os.path.isabs(config_path):
                # Assumes config_path is relative to the project root (elite_options_system/)
                # __file__ is elite_options_system/core/data_processing.py
                script_dir = os.path.dirname(os.path.abspath(__file__)) # .../core/
                project_root = os.path.dirname(script_dir) # .../
                abs_config_path = os.path.join(project_root, config_path) # .../config.json
...
try:
    from elite_options_system.core.strategies import IntegratedTradingSystem as ImportedITS # UPDATED
    RealIntegratedTradingSystem = ImportedITS
    logger.info("PROCESSOR: Successfully imported RealIntegratedTradingSystem from elite_options_system.core.strategies.")
except ImportError as import_error_its:
    logger.error(f"PROCESSOR IMPORT ERROR: Could not import IntegratedTradingSystem from elite_options_system.core.strategies: {import_error_its}")
...
class EnhancedDataProcessor:
    def __init__(self, config_path: str = DEFAULT_CONFIG_PATH_PROC, data_dir: str = DEFAULT_DATA_DIR_PROC):
        ...
        self.config_path: str = config_path # This path is relative to project root
        ...
        # Resolve processed_output_dir relative to project root (location of config.json)
        project_root_dir = os.path.dirname(os.path.abspath(self.config_path)) if os.path.isabs(self.config_path) else \
                           os.path.dirname(os.path.join(os.getcwd(), self.config_path))

        if os.path.isabs(output_dir_from_config):
            self.processed_output_dir = output_dir_from_config
        else:
            self.processed_output_dir = os.path.join(project_root_dir, output_dir_from_config)
        ...
        if elite_impact_module_available and EliteImpactCalculator and EliteConfig:
            try:
                ...
                elite_config_fields = {f.name for f in fields(EliteConfig)}
                elite_cfg_params_from_config = {k: v for k, v in calc_settings.items() if k in elite_config_fields}
                ...
                self.elite_calculator = EliteImpactCalculator(config=current_elite_config)
    ...
    def _load_main_config_for_processor(self) -> Dict[str, Any]:
        ...
        # config_path is now expected to be relative to project root, or absolute
        abs_config_path = self.config_path
        if not os.path.isabs(self.config_path):
            # data_processing.py is in elite_options_system/core/
    ...
    def _initialize_trading_system_instance(self) -> None:
        ...
        if RealIntegratedTradingSystem is not None:
            try:
                # Pass the already resolved absolute config path to ITS
                # Or ensure ITS also resolves config path from project root
                its_config_path = self.config_path # This is already project-root relative or absolute
                self.trading_system_instance = RealIntegratedTradingSystem(config_path=its_config_path)
                init_its_logger.info(f"Real ITS instance created with config: {its_config_path}.")
            except Exception as e_init_its_real:
                init_its_logger.error(f"Failed to instantiate real ITS: {e_init_its_real}", exc_info=True)
                init_its_logger.warning("Processor falling back to DUMMY ITS.")
                self.trading_system_instance = IntegratedTradingSystemDummy(config_path=self.config_path)
        else:
            init_its_logger.warning("Real ITS class not imported. Processor using DUMMY ITS.")
            self.trading_system_instance = IntegratedTradingSystemDummy(config_path=self.config_path)
    ...
    def _validate_input_data(self, options_chain_df: Optional[pd.DataFrame], symbol: str) -> Tuple[Optional[pd.DataFrame], Optional[str]]:
        ...
        required_base_cols = ["strike", "opt_kind", "symbol"]
        ...
        if self.processor_config.get("data_processor_settings",{}).get("perform_strict_column_validation", True):
            ...
            if any(f_col in missing_flow_cols for f_col in ['volm_buy','volm_sell','value_buy','value_sell']) and \
               any(f_col_bs in missing_flow_cols for f_col_bs in ['volm_bs','value_bs']):
    ...
    def _calculate_all_strike_level_flows(self, df_input: pd.DataFrame, symbol_context: str) -> pd.DataFrame:
        ...
        df, _ = self.trading_system_instance._ensure_columns(df, base_agg_cols, "BaseAggColsForFlows")
        ...
        greek_flow_map = {
            ...
            'theta': {'buy': 'thetas_buy', 'sell': 'thetas_sell', 'out_true_net': NET_THETA_EXPOSURE_COL}
        }
    ...
    def _package_results(self, symbol_str_pkg: str, fetch_ts_pkg: Optional[str], final_metric_rich_df_obj_pkg: pd.DataFrame, levels_dict_pkg: Dict[str, pd.DataFrame], signals_dict_pkg: Dict[str, Dict[str,list]], recommendations_list_pkg: List[Dict[str,Any]], underlying_data_pkg: Optional[Dict[str,Any]], volatility_data_pkg: Optional[Dict[str,Any]], atr_value_used_pkg: Optional[float], final_error_message_pkg: Optional[str], processor_config_snapshot_pkg: Dict[str,Any]) -> Dict[str, Any]:
    ...
    def process_data_with_integrated_strategies(self, options_chain_df: Optional[pd.DataFrame], underlying_data: Optional[Dict] = None, volatility_data: Optional[Dict] = None, historical_ohlc_df: Optional[pd.DataFrame] = None ) -> Dict[str, Any]:
        ...
        if isinstance(underlying_data,dict):
            ...
            fetch_ts_proc=underlying_data.get("fetch_timestamp")
             ...
             if "underlying_symbol" in options_chain_df.columns and pd.notna(options_chain_df["underlying_symbol"].iloc[0]):
                 ...
                 try: sym_proc=str(options_chain_df["symbol"].iloc[0]).upper().split(':')[0]
                 except: pass
             if "fetch_timestamp" in options_chain_df.columns and pd.notna(options_chain_df["fetch_timestamp"].iloc[0]):
                 try: fetch_ts_proc=str(options_chain_df["fetch_timestamp"].iloc[0])
                 except: pass
        ...
        if self.elite_calculator and isinstance(df_with_all_flows, pd.DataFrame) and not df_with_all_flows.empty:
            ...
            try:
                ...
                if current_price_for_elite is None and 'price' in df_with_all_flows.columns and not df_with_all_flows.empty:
                    current_price_for_elite = df_with_all_flows['price'].iloc[0]
                ...
                if isinstance(historical_ohlc_df, pd.DataFrame) and not historical_ohlc_df.empty and 'close' in historical_ohlc_df.columns:
                    if len(historical_ohlc_df) >= 21:
                        market_data_for_elite_calc = pd.DataFrame({
                            'price': historical_ohlc_df['close'],
                            'volatility': historical_ohlc_df['close'].pct_change().rolling(window=20).std() * np.sqrt(252)
                        })
                ...
                if current_price_for_elite is not None:
                    df_with_elite_calcs = self.elite_calculator.calculate_elite_impacts(
                        options_df=df_with_all_flows.copy(),
    ...
    def process_market_data(self, market_data_bundle_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        errors_list = [e for e in errors_list if e]
...
if __name__ == "__main__":
    ...
    test_cfg_path_main = DEFAULT_CONFIG_PATH_PROC # Uses corrected path
    if not os.path.exists(test_cfg_path_main):
         ...
         try:
             # Create dummy config in elite_options_system/config.json if it doesn't exist
             script_dir_main_test = os.path.dirname(os.path.abspath(__file__)) # .../core
             project_root_main_test = os.path.dirname(script_dir_main_test) # .../
             dummy_config_path_main_test = os.path.join(project_root_main_test, "config.json")

             with open(dummy_config_path_main_test,'w') as f_cfg_dum:
                 json.dump({"system_settings":{"log_level":"DEBUG"}, "data_processor_settings":{"perform_strict_column_validation":False}},f_cfg_dum)
             main_proc_test_logger.info(f"Created dummy '{dummy_config_path_main_test}' for test.")
             test_cfg_path_main = dummy_config_path_main_test # Use this path for the test instance
