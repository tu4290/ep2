...
class MSPIVisualizerV2:
    def __init__(self, config_path: Optional[str] = None, config_data: Optional[Dict[str, Any]] = None):
        ...
        if self.output_dir and isinstance(self.output_dir, str):
            try:
                # If output_dir is relative, it's relative to the main config file's location
                if not os.path.isabs(self.output_dir):
                    # Determine base path from full_app_config if possible, else from this script's dir as fallback
                    base_config_path = self.full_app_config.get("_config_file_path")
                    if base_config_path and os.path.isfile(base_config_path):
                        base_dir_for_output = os.path.dirname(base_config_path)
                        self.output_dir = os.path.normpath(os.path.join(base_dir_for_output, self.output_dir))
                    else: # Fallback if _config_file_path not in full_app_config
                        script_dir = os.path.dirname(os.path.abspath(__file__))
                        self.output_dir = os.path.normpath(os.path.join(os.path.dirname(script_dir), self.output_dir)) # Assuming visualizer.py is in core/
    ...
    def _load_visualizer_specific_config(self, config_path: Optional[str], full_app_config_from_init: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if full_app_config_from_init:
        ...
        elif config_path: # config_path here refers to the main app config (e.g. elite_options_system/config.json)
            abs_path_to_check = config_path
            if not os.path.isabs(config_path):
                # visualizer.py is in elite_options_system/core/
                # config.json is in elite_options_system/
                script_dir_viz = os.path.dirname(os.path.abspath(__file__)) # elite_options_system/core/
                project_root_dir = os.path.dirname(script_dir_viz) # elite_options_system/
                abs_path_to_check = os.path.join(project_root_dir, config_path) # elite_options_system/config.json
            ...
            try:
                if os.path.exists(abs_path_to_check):
                    ...
                    source_description_full_app = f"File ('{config_path}') resolved to '{abs_path_to_check}'"
                    # Store the path of the loaded config for relative path resolution later
                    temp_full_app_config_to_use["_config_file_path"] = abs_path_to_check
                else:
        ...
        if temp_full_app_config_to_use:
             self.full_app_config = temp_full_app_config_to_use # Store potentially loaded full config
    ...
    def _get_config_value(self, path: List[str], default_override: Any = None) -> Any:
        ...
        try:
            ...
            try:
                ...
                config_get_logger.debug(f"Found relative path '{'.'.join(relative_path_for_viz_config if 'relative_path_for_viz_config' in locals() else path)}' in self.config.")
                return current_level_viz
            except (KeyError, TypeError):
    ...
    def generate_sample_heatmap_data(strikes, dtes):
        """Generates a realistic, multi-dimensional dataframe for the heatmap."""
        data = []
        for dte in dtes:
            for strike in strikes:
                # Create some underlying structure
                gamma = np.exp(-((strike - 105) / 15)**2) * 100
                gamma -= np.exp(-((strike - 90) / 10)**2) * 120
                gamma *= (1 - dte/40) # Gamma decays closer to expiration

                # Add noise and randomness
                gamma += np.random.randn() * 5

                # Gamma Flow (Momentum)
                gamma_flow = np.random.randn() * 20 * (1 if dte < 10 else 0.5)

                # High Conviction Signal
                is_high_conviction = (abs(gamma) > 100 and np.random.rand() > 0.7)

                data.append({
                    'strike': strike,
                    'dte': dte,
                    'net_gamma_exposure': gamma,
    ...
    # --- Configuration ---
    STRIKES = np.arange(80, 125, 1)
    DTES = [0, 1, 2, 5, 10, 20, 30]
    heatmap_df = generate_sample_heatmap_data(STRIKES, DTES)

    # Pivot the data for the heatmap
    heatmap_pivot = heatmap_df.pivot(index='strike', columns='dte', values='net_gamma_exposure')
    flow_pivot = heatmap_df.pivot(index='strike', columns='dte', values='gamma_flow')
    conviction_pivot = heatmap_df.pivot(index='strike', columns='dte', values='is_high_conviction')

    # --- Visualization ---
    fig = go.Figure()

    # Layer 1 & 2: The Landscape with Divine Colors
    max_abs_gamma = heatmap_pivot.abs().max().max()
    divine_colorscale = [
        [0.0, 'rgba(220, 20, 60, 0.9)'],   # Crimson
        [0.5, 'rgba(20, 20, 20, 0.9)'],     # Charcoal Grey
        [1.0, 'rgba(0, 206, 209, 0.9)']     # Cyan/Teal
    ]
    ...
    fig.add_trace(go.Heatmap(
        ...
        colorscale=divine_colorscale,
        zmid=0,
        zmin=-max_abs_gamma,
        zmax=max_abs_gamma,
        name='Net Gamma',
        hovertemplate="<b>Strike: %{y} | DTE: %{x}</b><br>Net Gamma: %{z:,.0f}<extra></extra>",
        colorbar=dict(title='Net Gamma Exposure')
    ))

    # Layer 3: The Momentum Glyphs
    flow_symbols = np.where(flow_pivot.values > 0, 'triangle-up', 'triangle-down')
    flow_colors = np.where(flow_pivot.values > 0, 'lime', 'red')
    flow_sizes = np.abs(flow_pivot.values) / 5 + 5

    # We need to create a flat list of x, y, symbol, color, size for the scatter plot
    x_coords, y_coords = np.meshgrid(flow_pivot.columns, flow_pivot.index)
    fig.add_trace(go.Scatter(
        x=x_coords.flatten(),
        y=y_coords.flatten(),
        mode='markers',
        marker_symbol=flow_symbols.flatten(),
        marker_color=flow_colors.flatten(),
        marker_size=flow_sizes.flatten(),
        hoverinfo='none',
        name='Gamma Flow'
    ))

    # Layer 4: The Glow of Conviction
    conviction_y, conviction_x = np.where(conviction_pivot.values)
    fig.add_trace(go.Scatter(
        x=conviction_pivot.columns[conviction_x],
        y=conviction_pivot.index[conviction_y],
        mode='markers',
    ...
    # --- Final Layout ---
    fig.update_layout(
        title='<b>Gamma Exposure & Momentum Landscape</b>',
    ...
    fig.show()
        ...
        # THIS IS THE FINAL, CORRECTED CODE TO USE.
    # PASTE THIS INTO mspi_visualizer_v2.py IN PLACE OF THE OLD FUNCTION.
    def create_elite_impact_score_chart(
        ...
        **kwargs
        ...
        """
        TRANSFORMED by QUANTVIZ MASTER (v6 - The Redemption)
        Forged with a new divine palette, an intelligent custom legend, and hyper-robust
        data handling to ensure perfection for ALL symbols.
        """
        chart_name = "EliteImpactLandscape"
        full_chart_title = f"<b>{symbol.upper()}</b> - Elite Impact Landscape"
        chart_logger = self.instance_logger.getChild(chart_name)
        chart_logger.info(f"🎨 Forging a new masterpiece (v6 - Redemption): {full_chart_title}.")

        # --- 1. Configuration & Validation ---
        try:
            vis_cfg = self._get_config_value(["visualization_settings", "mspi_visualizer"], {})
            elite_chart_cfg = self._get_config_value(["elite_score_chart_config"], {})
            fig_height = vis_cfg.get("chart_specific_params", {}).get("component_comparison_height", 800)
            plotly_template = vis_cfg.get("plotly_template", "plotly_dark")
            price_filter_default = elite_chart_cfg.get("visual_settings", {}).get("price_range_filter_default_pct", 10.0)
            col_elite_score = "elite_impact_score"
            col_signal_strength = "signal_strength" # This is momentum
            col_confidence = "prediction_confidence"
            col_flow_type = "flow_classification"
        except Exception as e:
            chart_logger.error(f"Configuration loading failed: {e}", exc_info=True)
            return self._create_empty_figure(f"{symbol}-{chart_name}: Config Error", reason=str(e))

        # --- 2. Data Preparation ---
        if not isinstance(processed_data, pd.DataFrame) or processed_data.empty:
            return self._create_empty_figure(f"{symbol}-{chart_name}: No Data", height=fig_height)

        required_cols = [self.col_strike, col_elite_score, col_signal_strength, col_confidence, col_flow_type]
        df, _ = self._ensure_columns(processed_data.copy(), required_cols, chart_name)
        if df.empty: return self._create_empty_figure(f"{symbol}-{chart_name}: Missing Core Data", height=fig_height)

        df["strike_numeric"] = pd.to_numeric(df[self.col_strike], errors="coerce")
        agg_logic = {col: "mean" for col in [col_elite_score, col_signal_strength, col_confidence] if col in df.columns}
        agg_logic[col_flow_type] = 'first'
        plot_df = df.groupby("strike_numeric", as_index=False).agg(agg_logic).dropna(subset=[col_elite_score])
        if plot_df.empty: return self._create_empty_figure(f"{symbol}-{chart_name}: No Valid Data", height=fig_height)

        # --- 3. Price Range Filtering ---
        pct_to_use = selected_price_range_pct_override if isinstance(selected_price_range_pct_override, (int, float)) else price_filter_default
        if current_price and pct_to_use:
            min_s, max_s = current_price * (1 - pct_to_use / 100.0), current_price * (1 + pct_to_use / 100.0)
            plot_df = plot_df[(plot_df["strike_numeric"] >= min_s) & (plot_df["strike_numeric"] <= max_s)]
        if plot_df.empty: return self._create_empty_figure(f"{symbol}-{chart_name}: No Data in Range", height=fig_height)

        plot_df = plot_df.sort_values(by="strike_numeric", ascending=True).copy()
        ...
        divine_colorscale = [
            [0.0, 'rgba(220, 20, 60, 0.8)'],  # Crimson at min
            [0.5, 'rgba(45, 45, 45, 0.8)'],    # Charcoal Grey at zero
            [1.0, 'rgba(0, 206, 209, 0.8)']   # Dark Turquoise/Cyan at max
        ]
        max_abs_score = max(abs(plot_df[col_elite_score].min()), abs(plot_df[col_elite_score].max())) if not plot_df.empty else 1

        # --- 5. Forge the Masterpiece ---
        fig = go.Figure()
        fig.add_trace(go.Bar(
            y=plot_df['strike_str'], x=plot_df[col_elite_score], orientation='h',
            marker=dict(color=plot_df[col_elite_score], colorscale=divine_colorscale, cmin=-max_abs_score, cmax=max_abs_score),
            marker_opacity=plot_df['bar_opacity'], name='Elite Impact',
            customdata=plot_df[[col_confidence, col_signal_strength, col_flow_type]],
            hovertemplate="<b>Strike: %{y}</b><br><b>Score: %{x:.3f}</b><hr>Confidence: %{customdata[0]:.1%}<br>Momentum: %{customdata[1]:.2f}<br>Flow: %{customdata[2]}<extra></extra>",
            legendgroup="impact"
        ))
        fig.add_trace(go.Scatter(
            y=plot_df['strike_str'], x=plot_df[col_elite_score], mode='markers',
            marker=dict(size=plot_df['momentum_orb_size'], color=plot_df['momentum_orb_color'], symbol='circle', line=dict(width=2, color='rgba(0,0,0,0.7)')),
            name='Momentum/Flow', hoverinfo='none', legendgroup="momentum"
        ))

        # ** THE DIVINE GUIDE (CUSTOM LEGEND) **
        for flow_type, color in flow_color_map.items():
            fig.add_trace(go.Scatter(
                x=[None], y=[None], mode='markers',
                marker=dict(size=10, color=color, symbol='circle'),
                name=f'{flow_type} Flow', legendgroup='flow_types'
            ))

        # --- 6. Apply Divine Layout & THE LENS OF FOCUS ---
        title_text = f"<b>{symbol.upper()} Elite Impact Landscape</b>"

        # THE FIX FOR THE "ZOOMED OUT" PROBLEM
        min_score, max_score = plot_df[col_elite_score].min(), plot_df[col_elite_score].max()
        padding = (max_score - min_score) * 0.1 if (max_score > min_score) else abs(min_score) * 0.5 + 1
        x_axis_range = [min_score - padding, max_score + padding]

        fig.update_layout(
            title=dict(text=title_text, x=0.5), yaxis_title="Strike Price", xaxis_title="Elite Impact Score",
            template=plotly_template, height=fig_height, hovermode='y unified',
            plot_bgcolor='#111111', paper_bgcolor='#111111',
            yaxis=dict(showgrid=False, categoryorder='array', categoryarray=plot_df['strike_str'].tolist()),
            xaxis=dict(
                range=x_axis_range, # Apply the Divine Lens of Focus
                gridcolor='rgba(255, 255, 255, 0.1)', zeroline=True, zerolinecolor='rgba(255,255,255,0.3)'
            ),
            legend=dict(title_text="Flow Type", orientation='h', yanchor='bottom', y=1.02, xanchor='right', x=1)
        )

        if current_price:
            closest_strike_df = plot_df.iloc[(plot_df['strike_numeric'] - current_price).abs().argsort()]
            if not closest_strike_df.empty:
                closest_strike_str = closest_strike_df.iloc[0]['strike_str']
                fig.add_shape(type="line", yref="y", xref="paper", x0=0, y0=closest_strike_str, x1=1, y1=closest_strike_str, line=dict(color="#00BFFF", width=2, dash="dash"))
                fig.add_annotation(y=closest_strike_str, x=0.01, xref="paper", yshift=-15, text=f"Current: {current_price:.2f}", showarrow=False, font=dict(color="#00BFFF", size=12), bgcolor="rgba(0,0,0,0.7)", xanchor="left")

        chart_logger.info(f"🏆 Masterpiece Revealed (v6 - Redemption): {full_chart_title} is complete.")
        return fig
...
if __name__ == '__main__':
    ...
    if __name__ == '__main__':
        ...
        test_config_path = "../../config.json" # Adjusted path
        ...
        if not os.path.isabs(test_config_path):
            ...
            abs_test_config_path = os.path.normpath(os.path.join(script_dir_test_viz, test_config_path)) # Use normpath
