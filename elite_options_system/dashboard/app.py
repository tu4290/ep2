...
def _fallback_load_app_config_app(config_file_path: str = "config.json") -> Dict[str, Any]: # Updated default
    ...
    try:
        ...
        if not os.path.isabs(config_file_path):
            ...
            # Config is now in parent of dashboard directory
...
try:
    from elite_options_system.dashboard.layout import get_main_layout as imported_get_main_layout
    _layout_module_available_app = True
    get_main_layout_for_app: Callable[[], dbc.Container] = imported_get_main_layout # type: ignore
    print("ENH_DASH_APP: Successfully imported get_main_layout from elite_options_system.dashboard.layout.")
...
try:
    from elite_options_system.dashboard.callbacks import register_callbacks as imported_register_callbacks
    _callbacks_module_available_app = True
    register_callbacks_for_app: Callable[..., None] = imported_register_callbacks # type: ignore
    print("ENH_DASH_APP: Successfully imported register_callbacks from elite_options_system.dashboard.callbacks.")
...
try:
    from elite_options_system.dashboard.utils import load_app_config, get_config_value
    utils_load_app_config_func = load_app_config
    utils_get_config_value_func = get_config_value
    _utils_module_available_app = True
    print("ENH_DASH_APP: Successfully imported load_app_config and get_config_value from elite_options_system.dashboard.utils.")
...
try:
    from elite_options_system.dashboard.styling import APP_THEME as imported_app_theme
    _styling_module_available_app = True
    APP_THEME_FINAL: Any = imported_app_theme
    print(f"ENH_DASH_APP: Successfully imported APP_THEME from elite_options_system.dashboard.styling (Theme: {str(APP_THEME_FINAL)}).")
...
try:
    from elite_options_system.services.data_fetcher import EnhancedDataFetcher_v2
    from elite_options_system.services.tradier_fetcher import TradierDataFetcher
    from elite_options_system.core.data_processing import EnhancedDataProcessor
    from elite_options_system.core.strategies import IntegratedTradingSystem
    from elite_options_system.core.visualizer import MSPIVisualizerV2
...
# This logger will be used for messages from this app.py script.
...
# Config file is now expected to be in the parent directory of 'dashboard' (i.e., 'elite_options_system/config.json')
_default_config_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "config.json")
...
def cleanup_cache() -> None:
    ...
    dashboard_app_logger.info("Executing cleanup_cache function from elite_options_system.dashboard.app...")
    SERVER_CACHE.clear()
    COMPONENT_HISTORY_CACHE.clear()
    dashboard_app_logger.info("Server-side caches cleared.")

# --- Main Execution Block (for running with `python app.py` from `elite_options_system/dashboard/`) ---
if __name__ == "__main__":
    ...
    try:
        ...
        dashboard_app_logger.info(f"elite_options_system.dashboard.app direct run finished.")
